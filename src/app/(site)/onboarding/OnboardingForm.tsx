...
interface OnboardingFormProps {
  userName: string;
  defaultRole?: "buyer" | "producer" | null;
  foundingInviteValid?: boolean;
  inviteToken?: string;
...
export default function OnboardingForm({
  userName,
  defaultRole,
  foundingInviteValid = false,
  inviteToken,
  inviteError = null,
}: OnboardingFormProps) {
  const router = useRouter();
  const { update } = useSession();
  const [selected, setSelected] = useState<"buyer" | "producer" | null>(
    defaultRole ?? null,
  );
  ...
  return (
    <Card className="w-full max-w-lg border-border/60 bg-card/85 shadow-sm">
      <CardHeader className="text-center">
        ...
        <CardDescription>
          {foundingInviteValid
            ? "Your Founding Producer invitation is ready. Confirm your producer account to continue."
      ...
      <CardContent className="space-y-4">
        {error && (
          <div role="alert" aria-live="polite" className="rounded-md bg-destructive/10 p-3 text-sm text-destructive">
            <p>{error}</p>
            {error.toLowerCase().includes("waitlist") && (
              <Link href="/sell#waitlist" className="mt-2 inline-block font-medium underline">
                Join the waitlist
              </Link>
            )}
          </div>
